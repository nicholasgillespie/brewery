...
import schema from '../schemas/artisteSchema.js';
...
export default {
  ...
  async getOne(req, res, next) {
    ...
    // assemble query filter & options & execute
    const query = model.findOne('artistes', filter, { project });
    const result = await query;

    // treat result
    if (!result) return next(new AppError('No artiste found with that ID.', 400));
  ...
  async deleteOne(req, res, next) {
    ...
    const query = model.deleteOne('artistes', document);
    const result = await query;
